...
const useData = (apiUrl) => {
    ...
    useEffect(() => {
        ...
        const _fetchData = async () => { //ici les donnée qu'on veut actualiser toute les 5 secondes
         try {
            setLoading(true);
            //console.log(await axios.get(`${apiUrl}/api/3/history`));
          ...
          //console.log(await axios.get(`${apiUrl}/api/3/gpu`));
        }

        const _fetchCPU = async () => {
            let res=(await axios.get(`${apiUrl}/api/3/cpu`));
            setCPU(res.data);
        }
        const _fetchMemory = async () => {
            //ici ça tourne tout le temps en arriere plan
            let res=(await axios.get(`${apiUrl}/api/3/mem`));
            setMemory(res.data);
        }
        const _fetchIP = async () => {
            let res=(await axios.get(`${apiUrl}/api/3/ip`));
            setIP(res.data);
        }
        const _fetchQuickLook = async () => {
            let res=(await axios.get(`${apiUrl}/api/3/quicklook`));
            setQuickLook(res.data);
        }
        const _fetchSystemInfo = async () => {
            let res=(await axios.get(`${apiUrl}/api/3/system`));
            setSysInfo(res.data);
        }
        const _fetchUpTime = async () => {
            let res=(await axios.get(`${apiUrl}/api/3/uptime`));
            setUpTime(res.data);
        }
        const _fetchProcessCount = async () => {
            let res=(await axios.get(`${apiUrl}/api/3/processcount`));
            setSystemCount(res.data);
        }
        const _fetchSystemHour = async () => {
            let res= (await axios.get(`${apiUrl}/api/3/now`)); 
            setLocalHour(res.data);
        }
        const _fetchDiskio = async () => {
            let res= (await axios.get(`${apiUrl}/api/3/diskio`)); 
            setDiskIo(res.data);
        }
        const _fetchAmp = async () => {
            let res= (await axios.get(`${apiUrl}/api/3/amps`)); 
            setAmps(res.data);
        }
